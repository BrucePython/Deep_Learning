...
def mylinearregression():
    ...
    # 保存模型(保存的是权重的偏置)
    joblib.dump(lr,"myLinearRegression.pkl")
    """
    # 直接加载模型进行预测（不用再fit训练【测试集特征值】了）
    lr_local = joblib.load("myLinearRegression.pkl")

    print("预测未知的测试集结果", lr_local.predict(x_test))
    ...
    # 调用均方误差评估LinearRegression的结果误差:23; 平均每个样本的误差为：sqrt(23)，4~5
